...
export default function InnovativeSection() {
  ...
  return (
    <div
      className="sm:h-[120vh] h-fit flex sm:py-0 py-8 flex-col justify-center items-center w-full bg-white">
      <div className="w-[73%] flex flex-col justify-center items-center">
        ...
        <p
          className="sm:text-[17.5px] font-DM-sans sm:w-[60%] w-[100%] mt-[10px] text-dark-red font-normal tracking-wider space-x-5 sm:leading-[30px] text-[12px] leading-[17px]">
          {data.content}
        </p>
          ...
          <div className="absolute gap-[30px] inset-0 flex  justify-center items-center">
            {
              data.cards.map((card, i) => {
                return (
                  <button
                    key={i}
                    className=" sm:w-[150px] w-[150px] h-[40px]  sm:h-[60px]  border-black border-2" style={{ backgroundColor: (i === buttonIndex) ? "#8AC7C7" : '#fff' }} onClick={() => setButtonIndex(i)} >
                    {card.name}
                  </button>
                );
              })
            ...
            data.cards.map((card, i) => {
              return (
                <div
                  ...
                  className={(i == buttonIndex) ? `w-[100%] justify-evenly items-center mt-[20px] sm:flex-row flex-col flex` : `w-[100%] justify-evenly items-center mt-[20px] hidden`}>
                  <img
                    id={`rotatingImage3`}
                    src={card.image}
                  ...
                  <div className="sm:w-[60%] w-[100%] flex flex-col space-y-4 ">
                    <p className="text-light-pink sm:text-xl text-lg font-bold">{card.heading} </p>
