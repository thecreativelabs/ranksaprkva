...
const Button: React.FC<ButtonProps> = ({ children, className, size = "md", block, buttonStyle = "primary", ...props }) => {
  const getSizeClass = size === 'lg' ? 'px-6 py-3' : 'px-5 py-2';
  const getBlockClass = block ? 'w-full' : '';

  let styleClass = '';
  if (buttonStyle === 'outline') {
    styleClass = 'border-2 border-indigo-500 hover:bg-black text-black hover:text-white';
  } else if (buttonStyle === 'primary') {
    styleClass = 'bg-[#e55447] text-white border-2 border-transparent';
